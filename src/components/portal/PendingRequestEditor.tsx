        ...
        <div className="mt-2 space-y-3">
          ...
          {Array.from(new Set(serviceTypes.filter((s) => s.key !== "mold_swab" || selectedKeys.has(s.key)).map((s) => categoryKeyOf(s.key)))).map((c) => {
            ...
            return (
              ...
              // above it, now that it has no header of its own to mark the
              // category change.
              <div key={c} className={c === "lead" ? "mt-12" : undefined}>
                {/* Per Tim, 2026-09-02 — "the lead paint sampling title
                    should just be removed": Lead Paint Sampling is the only
                    item in this category, and the box itself now says that
                    (see serviceTypeDisplayLabel above), so the header would
                    just repeat it. */}
                {c !== "lead" && <div className="text-sm font-medium text-slate-700">{categoryLabelOf(c)}</div>}
