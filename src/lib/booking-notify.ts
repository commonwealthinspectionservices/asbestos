...
// notification every time someone pays me in Stripe" surfaced a real gap:
// a bank-transfer (ACH) payment doesn't finish instantly like a card does
// — Stripe puts the PaymentIntent into "processing" the moment the
// customer submits it, then takes another 3-5 business days to actually
// clear before invoice.paid ever fires. Until now that whole window was
// invisible — no notification exists until (and unless) it eventually
// succeeds. This is the "someone just started paying" half; the
// eventual invoice.paid still sends the existing "Payment received"
// email once it actually clears, so a bank-transfer payment now gets two
// emails (started, then cleared) where a card payment still gets one.
function paymentMethodLabel(paymentMethodTypes: string[]): string {
  if (paymentMethodTypes.includes("us_bank_account")) return "bank transfer (ACH)";
  if (paymentMethodTypes.includes("card")) return "card";
  return paymentMethodTypes[0] ?? "unknown method";
}

export async function sendJobPaymentPendingNotification(params: {
  jobId: string;
  amountCents: number;
  ...
  const { data: job } = await supabase
    .from("jobs")
    .select("project_number, service_address, customer_id")
    .eq("id", params.jobId)
    .maybeSingle();
  if (!job) return;
  ...
  const appUrl = getAppUrl();
  const jobUrl = appUrl ? `${appUrl}/admin/dashboard?jobId=${params.jobId}` : null;
  const method = paymentMethodLabel(params.paymentMethodTypes);

  const rows: [string, string][] = [
    ["Customer", customer?.company ? `${customer.name} (${customer.company})` : customer?.name ?? "Customer"],
  ...
  await sendEmail({
    to: process.env.OWNER_EMAIL!,
    subject: `Payment started (pending) — ${job.project_number ?? "job"}`,
    html: emailShell(`
      <p style="font-size:15px;">A payment was just started via ${escapeHtml(method)} — this takes a few business days to clear before it's actually final. You'll get a separate "Payment received" email once it does.</p>
      <table style="width:100%; font-size:14px; color:#16213a;">${tableRows}</table>
      ${jobUrl ? `<p style="margin-top:16px;"><a href="${jobUrl}" style="display:inline-block; background:#193466; color:#fff; padding:10px 16px; border-radius:8px; text-decoration:none; font-size:14px;">View job</a></p>` : ""}
    `),
  });
...
// The other half of the same visibility gap — a pending ACH payment can
// still fail days later (insufficient funds, closed account, etc.)
// instead of clearing. Without this, that failure was as invisible as the
// pending state itself used to be: no invoice.paid ever fires, so nothing
// ever told Tim the payment he was told about never actually arrived.
export async function sendJobPaymentFailedNotification(params: {
  jobId: string;
  amountCents: number;
  ...
  const rows: [string, string][] = [
    ["Customer", customer?.company ? `${customer.name} (${customer.company})` : customer?.name ?? "Customer"],
    ["Amount", formatCents(params.amountCents)],
    ["Reason", params.failureReason ?? "Not given by Stripe"],
    ["Address", expandAddress(job.service_address)],
  ];
  if (job.project_number) rows.unshift(["Project #", job.project_number]);
  const tableRows = rows
  ...
  await sendEmail({
    to: process.env.OWNER_EMAIL!,
    subject: `Payment failed — ${job.project_number ?? "job"}`,
    html: emailShell(`
      <p style="font-size:15px;">The pending payment you were notified about earlier did not go through.</p>
      <table style="width:100%; font-size:14px; color:#16213a;">${tableRows}</table>
      ${jobUrl ? `<p style="margin-top:16px;"><a href="${jobUrl}" style="display:inline-block; background:#193466; color:#fff; padding:10px 16px; border-radius:8px; text-decoration:none; font-size:14px;">View job</a></p>` : ""}
    `),
  });
